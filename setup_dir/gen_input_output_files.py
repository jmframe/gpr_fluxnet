...
with open(init_dir + 'obs.txt', 'r') as Observations_file, \
    ...
    for obs, out, forc in zip(observation_data, onestep_data, forcing_data):
        if float(obs[5]) > 0:
            ...
            print('something may be wrong with the observation record', obs)
        if float(out[10]) > 0:
            count['onestep'] = count['onestep'] + 1
        else:
            print('something may be wrong with the output record', out)
        if float(forc[10]) >= 0:
            count['forcing'] = count['forcing'] + 1
        else:
            print('something may be wrong with the forcing record', forc)
