...
!  REAL,    INTENT(IN), DIMENSION(ims:ime, jms:jme) :: tgxy
  ...
  REAL,    INTENT(OUT), DIMENSION(-NSNOW+1:NSOIL) :: ZSNSOXY
  REAL,    INTENT(OUT), DIMENSION(-NSNOW+1:0) :: TSNOXY
  REAL,    INTENT(OUT), DIMENSION(-NSNOW+1:0) :: SNICEXY
  REAL,    INTENT(OUT), DIMENSION(-NSNOW+1:0) :: SNLIQXY

!local
  INTEGER :: I,J,IZ
  REAL,                 DIMENSION(-NSNOW+1:    0) :: DZSNOXY
  REAL,                 DIMENSION(-NSNOW+1:NSOIL) :: DZSNSOXY
        ...
        ELSE
           ...
           ELSE IF ((SNODEP > 0.10) .AND. (SNODEP <= 0.25)) THEN
              ISNOWXY    = -2
              DZSNOXY(-1) = 0.05
              DZSNOXY( 0) = SNODEP - DZSNOXY(-1)
           ELSE IF ((SNODEP > 0.25) .AND. (SNODEP <= 0.35)) THEN
              ISNOWXY    = -3
              DZSNOXY(-2) = 0.05
              DZSNOXY(-1) = 0.5*(SNODEP-DZSNOXY(-2))
              DZSNOXY( 0) = 0.5*(SNODEP-DZSNOXY(-2))
           ELSE IF (SNODEP > 0.35) THEN
              ISNOWXY     = -3
              DZSNOXY(-2) = 0.05
              DZSNOXY(-1) = 0.10
              DZSNOXY( 0) = SNODEP - DZSNOXY(-1) - DZSNOXY(-2)
           END IF
        END IF
        ...
        DO IZ = ISNOWXY+1, 0
           TSNOXY(IZ)  = tgxy  ! [k]
           SNLIQXY(IZ) = 0.00
           SNICEXY(IZ) = 1.00 * DZSNOXY(IZ) * (SWE/SNODEP)  ! [kg/m3]
        END DO

        DO IZ = ISNOWXY+1, 0
           DZSNSOXY(IZ) = -DZSNOXY(IZ)
        END DO

        DZSNSOXY(1) = ZSOIL(1)
        DO IZ = 2,NSOIL
           DZSNSOXY(IZ) = (ZSOIL(IZ) - ZSOIL(IZ-1))
        END DO

        ZSNSOXY(ISNOWXY+1) = DZSNSOXY(ISNOWXY+1)
        DO IZ = ISNOWXY+2 ,NSOIL
           ZSNSOXY(IZ) = ZSNSOXY(IZ-1) + DZSNSOXY(IZ)
        ENDDO
